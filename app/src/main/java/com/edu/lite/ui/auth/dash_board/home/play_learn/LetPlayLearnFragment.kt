package com.edu.lite.ui.auth.dash_board.home.play_learn
            ...
            SimpleRecyclerViewAdapter(R.layout.rv_let_play_item, BR.bean) { v, m, pos ->
                when (v?.id) {
                    R.id.clLetPlay -> {
                        when (pos) {
                            0 -> {
                                val action = LetPlayLearnFragmentDirections.navigateToFeaturedQuizzesFragment(subject = args.subjectData.name.toString(), m.name.toString())
                                BindingUtils.navigateWithSlide(findNavController(), action)
                            }
                            1 -> {
                                val action = LetPlayLearnFragmentDirections.navigateToPracticeTestFragment()
                                BindingUtils.navigateWithSlide(findNavController(), action)
                            }
                            2 -> {
                                val action = LetPlayLearnFragmentDirections.navigateToVideoLessonsFragment()
                                BindingUtils.navigateWithSlide(findNavController(), action)
                            }
                            3 -> {
                                val action =
                                    LetPlayLearnFragmentDirections.navigateToCreativeProjectsFragment(subjectId = args.subjectData.name.toString(),grade = args.grade)
                                BindingUtils.navigateWithSlide(findNavController(), action)
                            }
                        }
                    }
