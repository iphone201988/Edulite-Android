package com.edu.lite.ui.dash_board.home.theams
...
class TodayThemeFragment : BaseFragment<FragmentTodayThemeBinding>() {
        ...
        viewModel.onClick.observe(viewLifecycleOwner) {
            when (it?.id) {
                ...
                R.id.btnStartMission -> {
                    val action = TodayThemeFragmentDirections.navigateToPickSubjectFragment(
                        subjectId = args.gradeId,
                        grade = args.grade
                    )
    ...
    private fun initOnQuestAdapter() {
        ...
        questAdapter.list = DummyList.questList(requireActivity())
