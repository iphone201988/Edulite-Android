...
class CustomBottomView @JvmOverloads constructor(
    ...
    // FAB gradient colors
    private var fabStartColor: Int = Color.WHITE
    private var fabEndColor: Int = Color.WHITE
    ...
    private val bottomControlX = fabRadius + 20 + (fabRadius / 2)
    private val bottomControlY = fabRadius / 4

    // total width of the curve
    private val curveHalfWidth = fabRadius * 2 + fabMargin + 22
    ...
    private fun setNavGradient() {
        navPaint.shader = LinearGradient(
    ...
    init {
        ...
        val a = context.theme.obtainStyledAttributes(attrs, R.styleable.CurvedBottomNavigationView, 0, 0)

        try {
            startColor = a.getColor(R.styleable.CurvedBottomNavigationView_cbn_startColor, Color.WHITE)
            endColor = a.getColor(R.styleable.CurvedBottomNavigationView_cbn_endColor, Color.WHITE)

            // FAB gradient colors
            fabStartColor = a.getColor(R.styleable.CurvedBottomNavigationView_cbn_fabStartColor, Color.WHITE)
            fabEndColor = a.getColor(R.styleable.CurvedBottomNavigationView_cbn_fabEndColor, Color.WHITE)

        //    fabBackgroundColor = a.getColor(R.styleable.CurvedBottomNavigationView_cbn_fabBg, Color.WHITE)
        } finally {
            a.recycle()
        }
        ...
        doOnLayout {
            // compute menu width etc
            menuCellWidth = width / cbnMenuItems.size
            val offsetX = menuCellWidth * selectedIndex
